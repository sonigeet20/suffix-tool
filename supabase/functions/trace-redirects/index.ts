...
async function fetchThroughAWSProxy(
  ...
  tracerMode?: string
): Promise<{ success: boolean; chain: any[]; proxy_ip?: string; geo_location?: any } | null> {
  try {
    const validModes = ['http_only', 'browser', 'anti_cloaking'];
    const effectiveMode = validModes.includes(tracerMode || '') ? tracerMode : 'http_only';
    console.log('🔄 Calling AWS Proxy Service:', awsProxyUrl, 'Mode:', tracerMode, '→', effectiveMode, 'Country:', targetCountry || 'any', 'Referrer:', referrer || 'none');
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { url, max_redirects = 20, timeout_ms = 60000, user_agent, user_id, use_proxy = true, target_country, referrer, tracer_mode = 'auto' } = await req.json() as TraceRequest;
    ...
    const userAgentStr = user_agent || 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36';
    ...
    if (effectiveUserId) {
      try {
        ...
        if (!settingsError && settings) {
          ...
          if (settings.luna_proxy_host && settings.luna_proxy_port && settings.luna_proxy_username && settings.luna_proxy_password) {
    ...
    if (effectiveUserId && !awsProxyUrl) {
      try {
        ...
        if (!providersError && providers && providers.length > 0) {
          ...
          console.log('✅ Selected provider:', selectedProvider.name, '(', selectedProvider.provider_type, ')');
        } else {
          console.log('ℹ️ No additional providers configured, using Luna from settings');
        }
      } catch (providerErr) {
    ...
    if (awsProxyUrl) {
      ...
      if (awsResult) {
        ...
        return new Response(
          JSON.stringify({
            success: false,
            error: 'AWS Proxy Service failed - no alternative tracing available',
          }),
          {
            status: 400,
            headers: {
      ...
      return new Response(
        JSON.stringify({
          success: false,
          error: 'No AWS Proxy Service configured - edge function requires AWS proxy for tracing',
        }),
        {
          status: 400,
          headers: {
    ...
    return new Response(
      JSON.stringify({
        success: false,
        error: 'Tracing via this edge function requires AWS Proxy Service configuration',
      }),
      {
        status: 400,
        headers: {
